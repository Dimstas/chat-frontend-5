...
export const LinksTab = ({ items }: LinksTabProps): JSX.Element => {
  ...
  const handleContactProfile = (item: MessageLinkApi): void => {
    router.push(`/contacts/${item.forwarded_in[0].uid}`);
    toggleInfoOpen();
  };
  return (
        ...
        {items?.map((item) => (
          <li key={item.message_id} className={styles.listItem}>
            {item.url.includes(baseUrl) && item.url.startsWith('http') ? (
              <div className={styles.link} onClick={() => handleContactProfile(item)}>
